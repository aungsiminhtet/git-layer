...
fn detect_framework(
    ...
    if let Some(framework) = detect_framework_from_hook_content(hook_content) {
        return framework;
    }
...
fn detect_framework_from_hook_content(hook_content: Option<&str>) -> Option<HookFramework> {
    let content = hook_content?;

    if content.contains("/_/husky.sh") {
        return Some(HookFramework::Husky);
    }

    if content.contains("generated by pre-commit") {
        return Some(HookFramework::PreCommit);
    }

    if content.contains("call_lefthook") {
...
mod tests {
    ...
    #[test]
    fn active_pre_commit_hook_content_beats_stale_repo_configs() {
        let (_tmp, ctx) = init_repo();
        ...
        .expect("failed to write lefthook config");
        std::fs::write(ctx.root.join(".pre-commit-config.yaml"), "repos: []\n")
        ...
        let inspection = inspect(&ctx).expect("inspection should succeed");
        assert_eq!(inspection.framework, HookFramework::PreCommit);
        assert!(
            matches!(
                inspection.health,
                GuardHealth::NeedsManualExternal {
                    framework: HookFramework::PreCommit
                }
            ),
            "expected NeedsManualExternal with Python pre-commit, got {:?}",
            inspection.health,
        );
    }

    #[test]
    fn husky_hook_path_beats_stale_repo_configs() {
        let (_tmp, ctx) = init_repo();
        ...
        let hook_dir = ctx.root.join(".husky");
        std::fs::create_dir_all(&hook_dir).expect("failed to create hook dir");
        std::fs::write(
            hook_dir.join("pre-commit"),
            "#!/usr/bin/env sh\necho husky\n",
        )
        .expect("failed to write husky hook");

        let inspection = inspect(&ctx).expect("inspection should succeed");
        assert_eq!(inspection.framework, HookFramework::Husky);
        assert!(
            matches!(
                inspection.health,
                GuardHealth::NeedsManualExternal {
                    framework: HookFramework::Husky
                }
            ),
            "expected NeedsManualExternal with Husky, got {:?}",
            inspection.health,
        );
    }

...
#[test]
fn guard_manual_guidance_prefers_active_pre_commit_hook_over_stale_lefthook_config() {
    let repo = init_repo();
    Command::new("git")
        .args(["config", "core.hooksPath", ".githooks"])
        .current_dir(repo.path())
    ...
    Command::new(assert_cmd::cargo::cargo_bin!("layer"))
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Python pre-commit"))
        .stdout(predicate::str::contains("pre-commit install"))
        .stdout(predicate::str::contains("lefthook.yml").not());
}
